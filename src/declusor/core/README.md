...
> [!NOTE]
> Interactive terminal components (console I/O and prompt loops) live in `presentation`. Concrete plugins live in the external `plugins/` directory and third-party packages.

## Modules

| Module   | Responsibility                                                                    |
| -------- | --------------------------------------------------------------------------------- |
| `parser` | CLI argument parsing, option normalization, and plugin registry binding           |
| `plugin` | Dynamic plugin discovery across tiers, contract validation, and plugin registries |
...
2. **Infrastructure Decoupling** — routing and parsing are completely decoupled from concrete plugin implementations.
